...
| Studio foundation | IMPLEMENTED | `wrestling_sim/studio/` | Standalone Vite internal development environment created. | Build and inspect in browser. |
| Studio ring viewer | NEEDS_LOCAL_TESTING | `studio/index.html`, `studio/app.js`, `studio/styles.css` | Shared runtime assets, 8 fps playback, frame stepping, push back, vertical staging, zoom, crowd, grid, and safe frame controls. | Run `npm install && npm run build`, then inspect desktop and mobile. |
...
| Studio deployment | PLANNED | `studio/README.md` | Existing Render service does not deploy the Studio because its root is `web_phaser`. | Use a separate internal deployment when approved. |

## Studio Module Matrix

| Module | Status | Scope |
|---|---|---|
| Hard Cam Ring Viewer | NEEDS_LOCAL_TESTING | Current ring, crowd, camera blocking, depth, safe frame, and 8 fps inspection. |
...
|---|---|---|
| Hard cam direction accepted | Browser review showing the correct side view camera | IN_PROGRESS |
| Ring pushed back accepted | Browser review at desktop and mobile widths | IN_PROGRESS |
| Foreground crowd accepted | Ring remains readable with crowd layer enabled | NEEDS_RENDER_TESTING |
| 8 fps ring cadence accepted | Two authored frames loop without distracting flicker | NEEDS_RENDER_TESTING |
