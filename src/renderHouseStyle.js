...
const STYLE = {
  ...
  teal: '#7ea4a0', green: '#789477', blue: '#93adb5', linen: '#efe7dc', dark: '#47423e', metal: '#bcc5c4', glass: '#a8d3db', terracotta: '#b66d55'
};
...
function drawFloorGrain(ctx, room) {
  ctx.save(); ctx.globalAlpha = .13; ctx.strokeStyle = '#8b6f53'; ctx.lineWidth = 1;
  for (let y = room.y + 36; y < room.y + room.h - 18; y += 28) {
    ctx.beginPath(); ctx.moveTo(room.x + 18, y); ctx.lineTo(room.x + room.w - 18, y + ((Math.floor(y) % 2) ? 3 : -2)); ctx.stroke();
  }
  ctx.restore();
}
...
function drawStyledDoorways(ctx, state, doorways) {
  for (const d of doorways.filter(x => x.floor === state.floor)) {
    ...
    ctx.fillRect(gap.x, gap.y, gap.w, gap.h);
    ctx.strokeStyle = '#7f756a'; ctx.lineWidth = 2; ctx.beginPath();
    if (d.w >= d.h) { const y = d.y + d.h / 2; ctx.moveTo(d.x + 8, y); ctx.lineTo(d.x + d.w - 8, y); ctx.moveTo(d.x + 8, y); ctx.arc(d.x + 8, y, Math.min(30, Math.max(18, d.w / 3)), -Math.PI / 2, 0); }
    else { const x = d.x + d.w / 2; ctx.moveTo(x, d.y + 8); ctx.lineTo(x, d.y + d.h - 8); ctx.moveTo(x, d.y + 8); ctx.arc(x, d.y + 8, Math.min(30, Math.max(18, d.h / 3)), 0, Math.PI / 2); }
...
function doorwayGapRect(d) {
  const alongPad = 14;
  const wallCut = 22;
  if (d.w >= d.h) { const h = Math.max(44, d.h + wallCut * 2); return { x: d.x - alongPad, y: d.y + d.h / 2 - h / 2, w: d.w + alongPad * 2, h }; }
...
function drawStyledWindows(ctx, state, windows) {
  ...
  for (const w of windows.filter(x => x.floor === state.floor)) {
    ...
    ctx.strokeStyle = '#f7f2e9'; ctx.lineWidth = 2;
...
function drawCouch(ctx, o) {
  ...
  const backY = up ? o.y + o.h - 20 : o.y + 3;
  const cushionY = up ? o.y + 9 : o.y + 21;
  rounded(ctx, o.x + 3, backY, o.w - 6, 17, 10, true, false, '#4e6662');
  rounded(ctx, o.x + 8, cushionY, o.w * .42, o.h - 30, 13, true, false, '#93b8b3');
  rounded(ctx, o.x + o.w * .52, cushionY, o.w * .40, o.h - 30, 13, true, false, '#93b8b3');
...
function drawTv(ctx, o, state) { rounded(ctx, o.x, o.y, o.w, o.h, o.wallMounted ? 3 : 5, true, false, '#3a3531'); rounded(ctx, o.x + 7, o.y + 5, o.w - 14, o.h - 10, 3, true, false, state.tv.on ? '#83bfc8' : '#262320'); if (o.wallMounted) { ctx.fillStyle = '#6b6258'; ctx.fillRect(o.x + o.w * .15, o.y - 4, o.w * .70, 4); } else { ctx.fillStyle = '#7b6c5a'; ctx.fillRect(o.x + o.w * .42, o.y + o.h, o.w * .16, 8); } }
