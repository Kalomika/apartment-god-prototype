...
function finishDeliverySafely(state, actor) {
...
export function updateDelivery(state, dt) {
  ...
  if (!actor) {
    finishDeliverySafely(state, null);
    return;
  }
  job.t -= dt;
  ...
  if (job.type === 'food') {
    ...
    if (job.phase === 'eating' && job.t <= 0) {
      ...
      finishDeliverySafely(state, actor);
    }
    return;
  }
  ...
  if (job.type === 'workout_gear') {
    ...
    if (job.phase === 'installing' && job.t <= 0) {
      ...
      log(state, 'Workout gear is installed in the living room.');
      finishDeliverySafely(state, actor);
    }
  }
}
...
export function buyWorkoutGear(state, actor) {
  ...
  if (state.delivery) {
    log(state, 'Another delivery is already in progress.');
    return false;
  }
