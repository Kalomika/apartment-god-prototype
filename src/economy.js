...
export function updateDelivery(state, dt) {
  ...
  if (job.phase === 'arriving' && job.t <= 0) {
    job.phase = 'exchange';
    job.t = job.type === 'workout' ? 5 : 4;
    job.x = 286;
    job.y = 622;
    job.bubble = 'DELIVERY';
    state.objectState.doorOpen = true;
    actor.action = job.type === 'workout' ? 'Receiving workout gear' : 'Receiving food delivery';
    actor.actionT = job.t;
...
export function buyWorkoutGear(state, actor) {
  ...
  if (state.delivery) return log(state, 'A delivery is already on the way.');
  ...
  setMood(actor, 'phone');
  say(actor, 'GYM');
  state.delivery = { type: 'workout', phase: 'arriving', actorId: actor.id, t: 12, x: 220, y: 630, floor: 0, bubble: 'GEAR' };
