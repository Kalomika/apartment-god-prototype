...
function drawPlayablePoolOverlay(ctx, o, state, active) {
  ...
  if (game) {
    drawPoolGameState(ctx, o, game);
  } else {
    drawRackedPoolTable(ctx, o);
  }
...
function drawRackedPoolTable(ctx, o) {
  ...
  for (let row = 0; row < 4; row++) {
    for (let col = 0; col <= row; col++) {
      circle(ctx, rackX + (col - row / 2) * gap, cy + row * gap, 5.4, colors[index++ % colors.length]);
    }
  }
}
...
function drawPortalMarker(ctx, o) {
  if (!o.styleAs && o.id !== 'garage_door') return;
  ctx.save();
  ctx.shadowColor = 'transparent';
  const doorColor = o.id.includes('garage') ? '#8ea3b7' : '#8b654c';
  rounded(ctx, o.x, o.y, o.w, o.h, 5, true, false, doorColor);
  rounded(ctx, o.x + 4, o.y + 4, Math.max(8, o.w - 8), Math.max(8, o.h - 8), 4, true, false, 'rgba(20,25,31,.45)');
  ctx.strokeStyle = '#f1c66a';
  ctx.lineWidth = 2;
  ...
  const label = o.id.includes('garage') ? 'GARAGE' : o.id.includes('back') || o.id.includes('yard') ? 'YARD' : 'DOOR';
  ctx.fillText(label, o.x + o.w / 2, o.y - 4);
  ctx.textAlign = 'left';
  ctx.restore();
}
