...
function drawSouthFacingFridge(ctx, o, state) {
  ctx.save();
  ctx.shadowColor = 'transparent';
  rounded(ctx, o.x, o.y, o.w, o.h, 8, true, false, '#f8f2e8');
  ctx.strokeStyle = '#bdb4aa';
  ctx.lineWidth = 2;
  rounded(ctx, o.x, o.y, o.w, o.h, 8, false, true);
  line(ctx, o.x + 8, o.y + o.h * .42, o.x + o.w - 8, o.y + o.h * .42, '#d5c9bc', 1.5);
  ctx.fillStyle = '#9fcbd3';
  ctx.fillRect(o.x + 10, o.y + o.h - 18, o.w - 20, 5);
  if (state.objectState.fridgeOpen) {
    rounded(ctx, o.x + 6, o.y + o.h - 3, o.w - 12, 42, 5, true, false, '#fffdf6');
    ctx.strokeStyle = '#d0c7bc';
    rounded(ctx, o.x + 6, o.y + o.h - 3, o.w - 12, 42, 5, false, true);
  }
  ctx.restore();
}

function drawDogBed(ctx, o, state, active) {
  ctx.save();
  ctx.shadowColor = 'transparent';
  rounded(ctx, o.x, o.y, o.w, o.h, 18, true, false, '#8f765f');
  rounded(ctx, o.x + 7, o.y + 7, o.w - 14, o.h - 14, 15, true, false, '#c8b7a1');
  rounded(ctx, o.x + 18, o.y + 13, o.w - 36, o.h - 25, 12, true, false, '#e1d1bd');
  if (active || hasAction(state, ['dog bed', 'dog rest'], o.floor)) {
...
function drawRackedPoolTable(ctx, o) {
  ...
  if (horizontal) {
    ...
    ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(rackX - 10, cy - 31); ctx.lineTo(rackX - 10, cy + 31); ctx.lineTo(rackX + 50, cy); ctx.closePath(); ctx.stroke();
    let index = 0;
    for (let row = 0; row < 4; row++) for (let col = 0; col <= row; col++) circle(ctx, rackX + row * gap, cy + (col - row / 2) * gap, 5.4, colors[index++ % colors.length]);
    return;
  }
  const cy = o.y + o.h * .62;
  ...
  let index = 0;
  for (let row = 0; row < 4; row++) for (let col = 0; col <= row; col++) circle(ctx, rackX + (col - row / 2) * gap, cy + row * gap, 5.4, colors[index++ % colors.length]);
}

function drawPoolGameState(ctx, o, game) {
  if (game.cueLine?.t > 0) { ctx.save(); ctx.globalAlpha = Math.min(1, game.cueLine.t); line(ctx, game.cueLine.x1, game.cueLine.y1, game.cueLine.x2, game.cueLine.y2, 'rgba(241,198,106,.86)', 2); ctx.restore(); }
  if (game.cueThrust?.t > 0) { const pct = Math.max(0, Math.min(1, game.cueThrust.t / .45)); const bx = game.cueThrust.x1 + (game.cueThrust.x2 - game.cueThrust.x1) * (1 - pct * .22); const by = game.cueThrust.y1 + (game.cueThrust.y2 - game.cueThrust.y1) * (1 - pct * .22); line(ctx, bx, by, game.cueThrust.x2, game.cueThrust.y2, '#d6b27a', 4); }
  for (const b of game.balls || []) { if (b.pocketed) continue; circle(ctx, b.x, b.y, b.id === 'cue' ? 6 : 6.5, b.fill || '#f8fbff'); if (b.id !== 'cue') circle(ctx, b.x - 1.2, b.y - 1.2, 2.1, 'rgba(255,255,255,.65)'); }
  const live = (game.balls || []).filter(b => !b.pocketed && b.id !== 'cue').length;
  ctx.fillStyle = '#f8fbff'; ctx.font = '900 8px system-ui'; ctx.fillText(`${live} BALLS`, o.x + 10, o.y + 11);
  if (game.mode === 'match') ctx.fillText(`SCORE ${Object.values(game.score || {}).join(' / ')}`, o.x + 10, o.y + o.h - 4);
}
