...
Last update: 2026-07-11 05:05 AM CT
...
| Vehicles and travel options | NEEDS_TESTING | `src/vehicleSystem.js`, `src/travelLocations.js`, `src/renderDynamic.js`, overlays | Vehicle polish pass improved SUV, sports convertible, bicycle, motorbike, ATV, active departures, returns, bicycle destinations, and travel cost or duration modifiers. | Test garage visuals, all vehicle menus, departure, return, passenger limits, bike hygiene and energy consequences. |
...
| Lived in activity object pass | NEEDS_TESTING | `src/world.js`, `src/config.js`, `src/renderObjects.js` | Bookshelf, coffee maker, dining table, and object animation overlays were added. Dining placement has also been adjusted. | Test object presence after reset, click menus, no path blocking. |
...
| Dog soccer ball play | NEEDS_TESTING | `src/soccerSystem.js`, `src/autoHooks.js`, `src/actions.js` | Soccer object menu now uses simple soccer ball practice. Dog play exists without scoring or scoreboard. | Test dog ball chase, no scoreboard, no crash from auto hook tick. |
...
| Dining table | Main floor | NEEDS_TESTING | Eat at table | Human | Plates, cups, seated eating | Placement may block movement | Test updated horizontal kitchen placement. |
...
| Soccer field or ball | Backyard | NEEDS_TESTING | Soccer ball practice, dog ball play | Human, dog | Ball and chase loop | Auto hook tick, no scoreboard | Test dog soccer ball play. |
| Family SUV | Garage | NEEDS_TESTING | Work, errand, mall, movies, date travel | Human passengers, dog possible | Parked SUV, active vehicle body | Departure and return logic | Test visual readability and return parking. |
| Sports convertible | Garage | NEEDS_TESTING | Work, errand, mall, movies, date travel | Human passengers | Parked convertible, active vehicle body | Passenger and route logic | Test visual readability and return parking. |
| Bicycle | Garage | NEEDS_TESTING | Work, errand, mall, movies, date travel | Limited human travel, no dog | Bike body, exposure consequence | Passenger limits, travel modifiers | Test menu, cost, speed, hygiene and energy effects. |
| Motorbike | Garage | NEEDS_TESTING | Work, errand, mall, movies, date travel | Limited human travel, no dog | Motorbike body, exposure consequence | Passenger limits, travel modifiers | Test menu, cost, speed, hygiene and energy effects. |
| ATV | Garage | NEEDS_TESTING | Existing vehicle support | Human future | Corrective vehicle overlay | Garage routing and readability | Test garage collision and readability. |
...
| Dog soccer chase | NEEDS_TESTING | Dog ball chasing without scoreboard | Yard direction | Loop | Soccer system ball chase | Test no scoring UI appears. |
...
| Dog soccer and auto hooks | Medium | Runtime tick hooks can crash or run too often if not guarded. | Test dog ball play and boot safety. |
