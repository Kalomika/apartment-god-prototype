...
Last update: 2026-07-11 05:25 AM CT
...
| Vehicles and travel options | NEEDS_CORRECTION | `src/vehicleSystem.js`, `src/travelLocations.js`, `src/renderDynamic.js`, overlays | Current July 10 vehicle polish is functionally useful but visually violates top down construction in places. Cars, bikes, motorbike, and ATV need top down reference correction before approval. | Correct side view wheel contamination, vehicle drawing conflicts, mounted rider pose, boarding, exiting, and parked versus active vehicle state. |
| Save and reset | NEEDS_TESTING | `src/saveSystem.js`, `src/state.js` | Reset and stale save protection are critical. New wardrobe, career, and relationship systems may need normalization. | Test reset after career, relationship, and wardrobe state exist. |
...
| Lived in activity object pass | NEEDS_TESTING | `src/world.js`, `src/config.js`, `src/renderObjects.js` | Bookshelf, coffee maker, dining table, and object animation overlays were added. Dining placement has also been adjusted but needs another placement correction. | Move dining table down one square and slightly right, then test pathing and kitchen clearance. |
...
| Dog soccer ball play | NEEDS_CORRECTION | `src/soccerSystem.js`, `src/autoHooks.js`, `src/actions.js` | Dog soccer must be ambient world behavior only. It must not show a mini game overlay, scoreboard, invisible menu, or player facing game screen unless Kam intentionally enters a future mode. | Hide dog soccer UI, slow frantic behavior, test dog ball chase without scoreboard or camera/UI interference. |
| Camera follow and room snapping | NEEDS_CORRECTION | `src/cameraNavigation.js`, `src/state.js`, travel and auto hook files | Current browser behavior appears to snap or shift rooms every few seconds without player intent. | Debug selected follow, offsite travel, dog soccer tick, locator state, and vehicle travel state. |
...
| Dining table | Main floor | NEEDS_CORRECTION | Eat at table | Human | Plates, cups, seated eating | Current placement is too close to kitchen objects | Move down one square and slightly right, then test pathing around table. |
...
| Soccer field or ball | Backyard | NEEDS_CORRECTION | Soccer ball practice, dog ball play | Human, dog | Ball and chase loop only | Auto hook tick, no scoreboard, no mini game overlay | Test dog soccer ball play as ambient world behavior. |
| Family SUV | Garage | NEEDS_CORRECTION | Work, errand, mall, movies, date travel | Human passengers, dog possible | True top down SUV, roof, hood, trunk, windshield, rear glass, subtle tire hints, mirrors, lights | Current vehicle overlay risks toy car and side wheel logic | Fix with reference based top down anatomy and boarding state. |
| Sports convertible | Garage | NEEDS_CORRECTION | Work, errand, mall, movies, date travel | Human passengers | True top down convertible body, cabin opening, windshield, rear deck, subtle tire hints | Current vehicle overlay risks toy car and side wheel logic | Fix with reference based top down anatomy and boarding state. |
| Bicycle | Garage | NEEDS_CORRECTION | Work, errand, mall, movies, date travel | Limited human travel, no dog | Top down frame, aligned front and rear wheels, seat, handlebars, mounted rider pose | Current bike risks side profile wheels and actor disappearing into frame | Show rider mounted on top with hands at bars. |
| Motorbike | Garage | NEEDS_CORRECTION | Work, errand, mall, movies, date travel | Limited human travel, no dog | Top down tires aligned front to back, seat, tank, handlebar, mounted rider pose | Current motorbike risks side profile wheels and actor disappearing into frame | Show rider mounted on top with hands at bars. |
| ATV | Garage | NEEDS_CORRECTION | Existing vehicle support | Human future | Real top down ATV body, seat, handlebars, four attached tire masses | Current pass reads like abstract black blocks or corner dots | Correct using top down ATV reference. |
...
| Dog soccer chase | NEEDS_CORRECTION | Dog ball chasing without scoreboard, mini game overlay, or frantic arcade behavior | Yard direction | Loop | Soccer system ball chase | Test no scoring UI, no game screen, no unwanted camera snap. |
...
| Vehicle door open and close | PLANNED | Character walks to door, unlock cue, door opens, character enters, door closes | Vehicle side based on parking and camera | Short transition | Actor currently pops into vehicle | Required before final vehicle approval. |
| Bicycle or motorbike mounting | NEEDS_CORRECTION | Actor mounts on top, hands align to handlebars, body aligns to seat and frame | Bike direction | Short transition plus riding loop | Actor appears to disappear into bike | Required before bike or motorbike approval. |
...
| Vehicle trip | Critical | NEEDS_CORRECTION | Confirm vehicle does not pop, duplicate, redraw parked after departure, or loop departure. Confirm active vehicle and parked vehicle states do not fight. |
| Vehicle top down compliance | Critical | NEEDS_CORRECTION | Inspect SUV, convertible, bicycle, motorbike, and ATV against Section 8. Reject side view wheels, black side boxes, toy proportions, symbolic wheel dots, or actor disappearing into bike. |
...
| Dining table placement | High | NEEDS_CORRECTION | Move dining table down one square and slightly right from current screenshot, then confirm kitchen clearance and pathing. |
| Dog soccer UI containment | Critical | NEEDS_CORRECTION | Dog soccer must show ambient world chase only. No scoreboard, mini game overlay, invisible menu, camera hijack, or player facing game screen unless Kam explicitly enters a future mode. |
| Camera snap stability | Critical | NEEDS_CORRECTION | Let the game run for several minutes on mobile. Confirm the camera does not snap or shift rooms without player input, selected actor travel, or explicit navigation. |
