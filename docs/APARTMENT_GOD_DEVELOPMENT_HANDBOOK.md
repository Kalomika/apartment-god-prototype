...
1. Verify repo.
2. Verify branch.
3. Read handbook, backup policy, no broad implementation rule, PNG upload fallback, idea logging rule, ongoing log, and development matrix.
...
- Log user intent before meaningful execution.

After editing:

1. Commit to the correct branch.
2. Update the log.
3. Update the Idea Bible or a clearly named append file.
4. Update the matrix or a clearly named matrix patch when runtime, visuals, gameplay, AI, objects, layout, controls, or tests changed.
5. If Kam needs browser testing, update `main` only after backup and explicit intent.
6. Tell Kam what branch was edited.
7. Give the test link and exact testing instructions.
