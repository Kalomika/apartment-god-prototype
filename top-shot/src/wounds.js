...
export function addBleed(state, fighter, severity, source = 'wound') {
  ...
  const safeSeverity = clamp(finiteOr(severity, 0), 0, 12);
  if (safeSeverity <= 0) return;
  const current = fighter.bleed || { rate: 0, pool: 0, bandaging: false, progress: 0, trailT: 0 };
  fighter.bleed = { rate: clamp(finiteOr(current.rate, 0) + safeSeverity, 0, 12), pool: clamp(finiteOr(current.pool, 0) + safeSeverity * 12, 0, 100), bandaging: false, progress: 0, source, trailT: 0 };
...
export function updateWounds(state, dt) {
  ...
  for (const f of state.fighters) {
    ...
    if (f.bleed.rate > 0) {
      const drain = f.bleed.rate * safeDt * (f.prone || f.wallLean ? 0.55 : 1);
      f.hp = clamp(finiteOr(f.hp, 100) - drain, 0, 100);
      f.bleed.pool = clamp(f.bleed.pool - drain * 1.8, 0, 100);
      f.bleed.trailT = finiteOr(f.bleed.trailT, 0) - safeDt;
      if (f.bleed.trailT <= 0) {
      ...
      if (f.hp <= 0) {
        ...
        f.defeated = true;
        f.bleed.bandaging = false;
        f.pose = 'blood_loss_kneel';
        f.stamina = 0;
        state.cinematic = { phase: 'outro', t: 0, loser: f.id, label: 'blood loss' };
        addLog(state, `${f.name} drops from blood loss.`);
      }
      if (f.bleed.pool <= 0) stopBleed(state, f, 'clotted');
    }
    if (f.bleed?.bandaging) updateBandage(state, f, safeDt);
  }
}
...
function updateBandage(state, fighter, dt) {
  ...
  fighter.bleed.progress = clamp(finiteOr(fighter.bleed.progress, 0) + dt * pace, 0, 1);
  fighter.stamina = clamp(finiteOr(fighter.stamina, 100) + dt * 3, 0, 100);
