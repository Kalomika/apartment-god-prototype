...
const ui = {
  ...
  cqcLabMode: document.getElementById('cqcLabMode'),
...
ui.ethos.addEventListener('change', () => {
  if (state.mode !== 'cqc') setCommanderEthos(state, ui.ethos.value);
});
...
document.querySelectorAll('[data-lab-action]').forEach(button => {
  button.addEventListener('click', () => {
    if (state.mode !== 'cqc') enterCqcLab(false);
    const action = button.dataset.labAction;
    if (action === 'slowmo') toggleCqcSlowMo(state);
    else triggerCqcAction(state, action);
    overlayMode = 'manual';
    ui.overlay.textContent = `CQC Lab: ${actionLabel(action)}.`;
  });
});
...
document.querySelectorAll('[data-drop]').forEach(button => {
  button.addEventListener('click', () => {
    if (state.mode === 'cqc') {
      overlayMode = 'manual';
      ui.overlay.textContent = 'Drops are disabled in CQC Lab. Use lab buttons to refine combat.';
      return;
    }
...
document.querySelectorAll('[data-command]').forEach(button => {
  button.addEventListener('click', () => {
    if (state.mode === 'cqc') {
      overlayMode = 'manual';
      ui.overlay.textContent = 'Main match commands are disabled in CQC Lab. Test moves one at a time.';
      return;
    }
    if (state.matchState !== 'running') {
      overlayMode = 'manual';
...
window.addEventListener('keydown', event => {
  ...
  if (key === 'l') {
    state.mode === 'cqc' ? enterMainMode(false) : enterCqcLab(false);
    event.preventDefault();
    return;
  }
  if (key === 'r' && state.mode === 'cqc') {
    triggerCqcAction(state, 'reset');
    event.preventDefault();
    return;
  }
  if (key !== 'c' || !world3D) return;
...
canvas.addEventListener('click', event => {
  if (state.mode === 'cqc') {
    overlayMode = 'manual';
    ui.overlay.textContent = 'CQC Lab is button-driven for now. Use Jab, Cross, Block, Parry, Slip, Step Back, or Reset.';
    return;
  }
...
function enterMainMode(updateUrl = true) {
  state = createBattle(ui.fighterA.value, ui.fighterB.value);
  lastRecordedKey = '';
  overlayMode = 'system';
  ui.overlay.textContent = `Main Match ready. ${cameraHelp}`;
  if (updateUrl) history.replaceState(null, '', window.location.pathname);
  updateUiMode();
}

function enterCqcLab(updateUrl = true) {
  state = createCqcLabState(ui.fighterA.value, ui.fighterB.value);
  lastRecordedKey = '';
  overlayMode = 'system';
  ui.overlay.textContent = 'CQC Lab loaded. Drone camera is tight overhead. Test one move at a time.';
  if (world3D?.setCameraMode) world3D.setCameraMode('topdown');
  if (updateUrl) history.replaceState(null, '', `${window.location.pathname}?mode=cqc`);
  updateUiMode();
}

function updateUiMode() {
  const isLab = state.mode === 'cqc';
  document.body.classList.toggle('cqc-mode', isLab);
  ui.labControls?.classList.toggle('hidden', !isLab);
  ui.mainMode?.classList.toggle('active-mode', !isLab);
  ui.cqcLabMode?.classList.toggle('active-mode', isLab);
  ui.start.textContent = isLab ? 'Reset Lab' : state.matchState === 'ready' ? 'Begin Sortie' : 'Restart Sortie';
  ui.pause.textContent = state.paused ? 'Resume' : 'Pause';
}
...
function frame(now) {
  ...
  last = now;
  if (state.mode === 'cqc') updateCqcLab(state, dt);
  else updateBattle(state, dt);
  saveFinishedMatchRecord();
...
function renderMatchOverlay() {
  if (state.mode === 'cqc') {
    if (overlayMode === 'system') ui.overlay.textContent = `CQC Lab, ${formatTime(state.clock)}. Test moves one at a time.`;
    return;
  }
  if (state.matchState === 'ready') {
    overlayMode = 'system';
...
function renderDomHud() {
  const cards = state.fighters.map(f => {
    ...
    const request = state.mode === 'cqc' ? null : fighterRequest(state, f);
    const status = fighterStatus(f);
    return `<article class="fighter-card"><h3><span>${escapeHtml(f.name)}</span><small>${escapeHtml(stage.label)}</small></h3>${requestHtml(request)}<div class="action-pill">${escapeHtml(status.label)}</div><small class="action-detail">${escapeHtml(status.detail)}</small>${bar('HP', f.hp)}${bar('Stamina', f.stamina)}${bar('Dodge', f.dodge)}${bar('Block', f.block)}<small>${escapeHtml(fighterShortState(f))}</small></article>`;
  }).join('');
...
function commentaryCard() {
  const recent = (state.log || []).slice(0, 5).map(item => `<li>${escapeHtml(item)}</li>`).join('');
  return `<article class="fighter-card commentary-card"><h3>Fight Commentary</h3><ol class="mini-log">${recent || '<li>No action yet.</li>'}</ol></article>`;
}

function mainMatchCard(drops) {
  return `<article class="fighter-card"><h3>Commander</h3>${bar('Trust', state.trust)}<small>Match Time: ${formatTime(state.clock)}</small><br><small>Ethos: ${escapeHtml(state.commanderEthos)}</small><br><small>${escapeHtml(state.result || state.matchState)}</small><br><small>Map: desert industrial test site</small><br><small>${cameraHelp}</small><br>${drops}</article><article class="fighter-card"><h3>Terrain</h3><small>Boulders, cover, shadows, pipes, tanks, raised catwalk, and separated collision volumes are live.</small></article>`;
}

function labCard() {
  return `<article class="fighter-card"><h3>CQC Lab</h3><small>Lab Time: ${formatTime(state.clock)}</small><br><small>Mode: one move at a time</small><br><small>Camera: close overhead drone</small><br><small>Last Action: ${escapeHtml(actionLabel(state.lab?.action || 'guard'))}</small><br><small>${state.lab?.slowMo ? 'Slow motion is on.' : 'Slow motion is off.'}</small></article>`;
}

function recordsCard() {
  const top = Object.values(records.fighters || {}).sort((a, b) => (b.wins - b.losses) - (a.wins - a.losses)).slice(0, 4);
  const last = records.matches?.[0];
  const standings = top.map(r => `<li>${escapeHtml(r.name)}: ${r.wins}-${r.losses}</li>`).join('') || '<li>No completed matches yet.</li>';
  const lastLine = last ? `<small>Last: ${escapeHtml(last.winner)} over ${escapeHtml(last.loser)} in ${formatTime(last.length)} by ${escapeHtml(last.method)}.</small>` : '<small>No result recorded yet.</small>';
  return `<article class="fighter-card records-card"><h3>Records</h3>${lastLine}<ol class="mini-log">${standings}</ol></article>`;
}

function saveFinishedMatchRecord() {
  if (state.mode === 'cqc' || state.matchState !== 'finished' || !state.result) return;
  const key = `${state.result}|${Math.floor(state.clock)}`;
  if (lastRecordedKey === key) return;
  lastRecordedKey = key;
  const active = state.fighters.filter(f => !f.incapacitated && !f.extracted);
  const winner = active[0] || state.fighters.find(f => state.result.includes(f.name));
  const loser = state.fighters.find(f => f !== winner) || state.fighters[1];
  const record = {
    at: new Date().toISOString(),
    length: Math.floor(state.clock),
    winner: winner?.name || 'Unknown',
    loser: loser?.name || 'Unknown',
    method: inferMethod(state),
    highlights: (state.log || []).slice(0, 8)
  };
  records.matches.unshift(record);
  records.matches = records.matches.slice(0, 50);
  addFighterRecord(record.winner, 'win');
  addFighterRecord(record.loser, 'loss');
  saveRecords(records);
}

function addFighterRecord(name, result) {
  if (!name) return;
  records.fighters[name] ||= { name, wins: 0, losses: 0 };
  if (result === 'win') records.fighters[name].wins += 1;
  else records.fighters[name].losses += 1;
}

function inferMethod(matchState) {
  const result = String(matchState.result || '').toLowerCase();
  if (result.includes('extracted') || result.includes('forfeit')) return 'extraction forfeit';
  if (result.includes('incapacitated')) return 'incapacitation';
  if (result.includes('out')) return 'stoppage';
  return 'decision';
}
...
function fighterStatus(f) {
  ...
  if (pose.includes('hit')) return { label: 'Hit Stun', detail: 'Reacting to impact and giving ground.' };
...
function fighterShortState(f) {
  const parts = [f.intent || 'idle', f.pose || 'guard'];
  if (f.memory?.command?.type) parts.push(`command ${f.memory.command.type}`);
  return parts.join(' / ');
}
...
function actionLabel(action) {
  return String(action || 'guard').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
}
...
function drawFallbackTerrain(ctx, fallbackState) {
  ...
  ctx.fillText(state.mode === 'cqc' ? 'TOP SHOT CQC LAB' : 'TOP SHOT', 24, 42);
...
function loadRecords() {
  try {
    ...
    return { matches: Array.isArray(parsed.matches) ? parsed.matches : [], fighters: parsed.fighters || {} };
  } catch {
    return { matches: [], fighters: {} };
  }
}

function saveRecords(nextRecords) {
  try { localStorage.setItem(RECORD_KEY, JSON.stringify(nextRecords)); } catch {}
}
