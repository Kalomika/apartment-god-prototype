...
function updateDeployment(state, dt) {
  ...
  for (const f of state.fighters) {
    ...
    if (f.deploy.t < delay) {
      allLanded = false;
      f.x = f.deploy.fromX; f.y = f.deploy.fromY; f.deployAltitude = f.deploy.altitude;
      f.pose = 'parachute_wait'; f.intent = 'stacking';
      continue;
    }
...
function updateFighter(state, f, dt) {
  ...
  const movingPose = ['walk', 'run', 'crouchWalk', 'rush', 'stagger_limp', 'limp_run', 'careful_walk', 'wall_strafe', 'roll', 'combat_roll', 'combat_dive', 'climb_up', 'jump_down_escape', 'drop_down'].includes(f.pose);
...
function stuckEscapePoint(state, f, destination) {
  ...
  const options = [];
  for (const radius of [92, 132, 170]) for (const a of angles) { const p = { x: clamp(f.x + Math.cos(a) * radius, 72, 888), y: clamp(f.y + Math.sin(a) * radius, 72, 648) }; if (!blocked(state.arena, p, 18)) options.push(p); }
...
function commandedDestination(state, f, enemy) {
  ...
  if (['ranged', 'projectile', 'grenade'].includes(command.type)) { const desired = command.type === 'grenade' ? 250 : 175; const away = Math.atan2(f.y - enemy.y, f.x - enemy.x); return { x: f.x + Math.cos(away) * desired, y: f.y + Math.sin(away) * desired }; }
...
function nearestUsefulPickup(state, f) {
  ...
  const pick = [...items].sort((a, b) => dist(f, a) - dist(f, b))[0];
