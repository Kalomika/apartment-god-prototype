...
const MOVES = [
  ...
  { id: 'roundhouse', limb: 'rightLeg', kind: 'power', reach: 60, damage: 17, stamina: 18, knock: 44, lean: -1 },
  { id: 'headbutt', limb: 'head', kind: 'headbutt', reach: 28, damage: 10, stamina: 12, knock: 16, lean: 0 }
];
...
export function updateCombat(state, dt) {
  for (const f of state.fighters) {
    ...
    f.stamina = clamp(f.stamina + dt * (f.hidden ? 11 : 5), 0, 100);
...
function fireGun(state, f, enemy) {
  ...
  if (!shots) return;
  const weapon = rifle ? 'rifle burst' : 'pistol';
  ...
  addLog(state, `${f.name} fires ${weapon}.`);
}

function hitSpread(f, enemy) { return (f.stats.aim / 100) * (enemy.dodge < 30 ? 0.2 : 0.11); }
function rayEnd(state, f, a, range) {
  for (let t = 22; t < range; t += 12) {
    const p = { x: f.x + Math.cos(a) * t, y: f.y + Math.sin(a) * t };
    if (blocked(state.arena, p, 2)) return p;
  }
  return { x: f.x + Math.cos(a) * range, y: f.y + Math.sin(a) * range };
}

function throwProjectile(state, f, enemy, type, speed, damage, resource) {
  f.resources[resource]--;
  const a = angleTo(f, enemy) + rand(-0.08, 0.08);
  state.projectiles.push({ type, team: f.team, owner: f.name, x: f.x, y: f.y, vx: Math.cos(a) * speed, vy: Math.sin(a) * speed, damage, ttl: 2.2, stuck: false });
  f.rangedCd = type === 'arrow' ? 1.15 : 0.75; f.cooldown = 0.25; f.pose = type;
  addLog(state, `${f.name} throws ${type}.`);
}
...
function melee(state, f, enemy) {
  ...
  f.pose = move.id; f.currentMove = { ...move, ttl: 0.22 };
...
function defend(state, attacker, defender, move) {
  const dodgeChance = defender.dodge / 130 * (defender.stats.dodge / 100);
  if (defender.dodge > 18 && chance(dodgeChance)) { defender.dodge -= 22; defender.pose = 'dodge'; shove(defender, angleTo(attacker, defender), 18); return 'dodge'; }
  const blockLimb = chooseBlockLimb(defender, move);
  const limb = defender.limbs[blockLimb];
  const blockChance = defender.block / 135 * (defender.stats.block / 100) * (limb.guard / 100);
  if (defender.block > 14 && limb.guard > 12 && chance(blockChance)) {
    defender.block -= 18; limb.guard -= 22 + move.damage * 0.65; limb.t = 0.18; defender.pose = `block_${blockLimb}`;
    if (attacker.stats.counter < defender.stats.counter && chance(defender.stats.counter / 310)) counter(state, defender, attacker);
    return 'block';
  }
  return 'hit';
}

function chooseBlockLimb(defender, move) {
  if (move.kind === 'kick') return move.limb === 'leftLeg' ? 'rightLeg' : 'leftLeg';
  return move.limb === 'leftArm' ? 'rightArm' : 'leftArm';
}
...
function grapple(state, f, enemy) {
  f.fight -= 22; f.cooldown = 1.1; enemy.pose = 'thrown'; f.pose = 'grapple';
  const a = angleTo(f, enemy); shove(enemy, a, 78); hit(state, f, enemy, 9, 'throw', a); state.effects.push({ type: 'grapple', x: enemy.x, y: enemy.y, ttl: EFFECT_TTL.grapple });
