...
export function updateCombat(state, dt) {
  for (const f of state.fighters) {
    ...
    f.peekCooldown = Math.max(0, (f.peekCooldown || 0) - dt);
    f.heat = Math.max(0, f.heat - dt * (f.hidden || f.coverPinned ? 25 : 13));
    f.stamina = clamp(f.stamina + dt * (f.hidden || f.wallLean || f.coverPinned ? 14 : 5), 0, 100);
    f.fight = clamp(f.fight + dt * 6, 0, 100);
    f.dodge = clamp(f.dodge + dt * (isSuppressed(state, f) ? 18 : 8), 0, 100);
    f.block = clamp(f.block + dt * (isSuppressed(state, f) ? 18 : 9), 0, 100);
...
export function tryAttack(state, f, enemy, visible) {
  ...
  if ((f.archetypeId === 'ninja' || f.archetypeId === 'shadow_ninja') && f.specialCd <= 0 && f.resources.smoke > 0 && (isSuppressed(state, f) || visible && d > 92 || f.hp < 70)) return smoke(state, f, enemy);
...
function fireGun(state, f, enemy) {
  ...
  const shots = rifle ? Math.min(f.coverPinned ? 2 : 1, f.resources.rifle) : f.resources.pistol > 0 ? 1 : 0;
  if (!shots) return;
  f.peekT = 0.32;
...
function updateProjectiles(state, dt) {
  for (const p of state.projectiles) {
    ...
    const target = state.fighters.find(f => f.team !== p.team && !f.incapacitated && !f.defeated && !f.extracted && (f.rollT || 0) <= 0 && dist(f, p) < 24);
    if (target) { hit(state, { name: p.owner, stats: { aim: 55 } }, target, p.damage * 0.72, p.type, Math.atan2(p.vy, p.vx)); p.ttl = 0; }
  }
...
function projectileSurfaceImpact(state, p, surface) {
  ...
  if ((p.type === 'arrow' || p.type === 'shuriken') && material === 'metal') { p.vx *= -0.35; p.vy *= -0.35; p.ttl = Math.min(p.ttl, 0.45); state.effects.push({ type: 'spark', x: p.x, y: p.y, angle, ttl: 0.24, material }); impact(state, p.x, p.y, 'bounce'); return; }
...
function defend(state, attacker, defender, move) { const side = attackSide(move); const normalLimb = normalDefenseLimb(move, side); const crossLimb = crossDefenseLimb(move, side); const normal = defender.limbs[normalLimb]; const cross = defender.limbs[crossLimb]; const pressure = clamp((defender.stats.block + defender.stats.dodge + defender.stats.counter) / 300, 0.2, 1.05); if (canSlip(move) && defender.dodge > 18 && chance(defender.dodge / 155 * defender.stats.dodge / 100)) { defender.dodge -= 18; defender.pose = `slip_${side}`; shove(defender, angleTo(attacker, defender) + (side === 'left' ? 0.45 : -0.45), 16); maybeCounter(state, defender, attacker, pressure * 0.45); return 'slip'; } if (normal && defender.block > 16 && normal.guard > 16 && canParry(move) && chance(defender.stats.counter / 190 * pressure)) { defender.block -= 12; normal.guard -= 13 + move.damage * 0.35; normal.t = 0.16; defender.pose = `parry_${normalLimb}`; maybeCounter(state, defender, attacker, pressure * 0.75); return 'parry'; } if (normal && defender.block > 14 && normal.guard > 12 && chance(defender.block / 135 * defender.stats.block / 100 * normal.guard / 100)) { defender.block -= 18; normal.guard -= 22 + move.damage * 0.65; normal.t = 0.2; defender.pose = `block_${normalLimb}`; maybeCounter(state, defender, attacker, pressure * 0.28); return 'block'; } if (cross && defender.block > 28 && cross.guard > 24 && !['kick', 'knee', 'power'].includes(move.kind) && chance(defender.block / 185 * defender.stats.block / 100 * cross.guard / 100)) { defender.block -= 28; cross.guard -= 32 + move.damage * 0.7; cross.t = 0.22; defender.pose = `cross_block_${crossLimb}`; maybeCounter(state, defender, attacker, pressure * 0.4); return 'cross_block'; } return 'hit'; }
...
function hit(state, attacker, defender, amount, type, a, move = null) { const armor = type === 'shot' && ['survival_commando', 'marine'].includes(defender.archetypeId) && chance(0.22); const toughness = defender.stats?.toughness || 60; const defense = defender.wallLean || defender.shadowHidden || defender.crouch || defender.prone ? 0.62 : isSuppressed(state, defender) ? 0.78 : 1; const final = Math.max(1.1, amount * (1.04 - toughness / 240) * defense * (armor ? 0.42 : 1)); defender.hp = clamp(defender.hp - final, 0, 100); updateVitalityCap(defender); defender.memory.lastHitBy = attacker.name; defender.pose = armor ? 'armor_hit_react' : type === 'shot' ? 'shot_react' : 'hit'; defender.hitLean = move?.lean || (Math.cos(a) > 0 ? 1 : -1); defender.woundT = Math.max(defender.woundT || 0, type === 'shot' ? 0.9 : 0.55); shove(defender, a, final * 0.34); if (armor) state.effects.push({ type: 'spark', x: defender.x, y: defender.y, angle: a, ttl: 0.2, material: 'armor' }); else if (['shot', 'sword', 'knife', 'arrow_stab', 'arrow', 'shuriken'].includes(type)) state.effects.push({ type: 'blood_spray', x: defender.x, y: defender.y, angle: a, ttl: 0.32, source: type }); suppressTarget(state, defender, attacker, defender); if (!armor && ['shot', 'sword', 'knife', 'arrow_stab', 'arrow', 'shuriken'].includes(type) && final > 5 && chance(type === 'shot' ? 0.42 + final / 85 : 0.16 + final / 120)) addBleed(state, defender, type === 'shot' ? 1.9 : type === 'sword' ? 1.8 : 1.15, type); if (canSuddenIncapacitate(attacker, defender, type, final)) return incapacitate(state, defender, attacker, type); if (defender.hp <= 0) defeat(state, defender, attacker); }
