...
function shouldRanged(f, d) { if (f.rangedCd > 0 || f.heat > 88) return false; if (['cqc', 'disarm', 'sword'].includes(f.memory.command?.type)) return false; if (f.memory.command?.type === 'projectile') return d > 60; if (['marine', 'suit_operative', 'survival_commando'].includes(f.archetypeId)) return (f.resources.rifle > 0 || f.resources.pistol > 0) && d > (f.archetypeId === 'suit_operative' ? 75 : 95); if (f.archetypeId === 'ninja') return f.resources.shuriken > 0 && d > 92 && d < 430; if (f.archetypeId === 'archer') return f.resources.arrows > 0 && d > 80 && d < 520; if (f.archetypeId === 'martial_artist') return f.resources.debris > 0 && d > 100 && d < 310; return false; }
function ranged(state, f, enemy) { if (['marine', 'suit_operative', 'survival_commando'].includes(f.archetypeId)) return fireGun(state, f, enemy); if (f.archetypeId === 'ninja') return throwProjectile(state, f, enemy, 'shuriken', 520, 9, 'shuriken'); if (f.archetypeId === 'archer') return throwProjectile(state, f, enemy, 'arrow', 420, 15, 'arrows'); return throwProjectile(state, f, enemy, 'debris', 360, 7, 'debris'); }
