...
| WF-003 | Backup branch | Maintain rollback branch before risky or Starshot work. | Baseline | `backup/top-shot-v0-1-2026-07-11-coverage-matrix` and `backup/top-shot-coverage-matrix-2026-07-11-smoke-fix` for recent passes | branch refs, handoff | Confirm backup branch exists before risky work. | High | Create a fresh dated backup when stable moves. |
...
| WF-005 | Open Top Shot v0.1 PR | Main Top Shot prototype PR remains open and draft. | Known issue | PR #5, `top-shot-v0-1`; mitigation branch `top-shot-smoke-invalid-state-fix` | full `top-shot/` folder, `src/explosives.js`, `tests/simSmoke.js` | `npm run smoke` currently has documented failure in PR body, mitigation branch now adds diagnostics and hardening. | High | Run smoke on mitigation branch, then update blocker status. |
...
| WF-008 | Smoke invalid-state fix | Harden dive and grenade simulation against NaN fighter/projectile state. | Needs verification | `top-shot-smoke-invalid-state-fix` | `src/explosives.js` | `node --check` passed on patched file; full smoke still needed. | High | Run `npm run check`, `npm run smoke:sim`, `npm run smoke`, and `npm run build` from `top-shot/`. |
| WF-009 | Smoke diagnostics split | Keep the full smoke gate but split named scripts and enrich failure output. | Needs verification | `top-shot-smoke-invalid-state-fix` | `package.json`, `tests/simSmoke.js` | `npm run smoke:sim` gives matchup, phase, tick, fighter, projectile, and log context when state breaks. | High | Run split scripts locally or in CI. |
| WF-010 | Whole-game anomaly audit | Audit docs, build scripts, smoke tests, CQC, stealth, AI/movement/combat helpers, Three.js presentation, playable links, and core runtime entry points for missed instructions. | Needs verification | `top-shot-anomaly-audit-2026-07-12` | `docs/AUDIT_REPORT_2026-07-12.md`, docs, `tests/cqcSmoke.js`, `tests/stealthSmoke.js` | Audit report exists, docs encode playable-link rule, CQC and stealth smoke guard preserved behavior without stale/random assumptions. | High | Run strengthened smoke commands and patch any true failures. |
| WF-011 | Studio visual pipeline PR | First visual-pipeline slice reports a stealth smoke blocker. | PR open | PR #26 | studio pipeline branch files, `tests/stealthSmoke.js` | Visual pipeline work must pass corrected smoke expectations before merge. | Starshot | Rebase or port the corrected stealth smoke expectation into PR #26 if needed. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| GP-001 | Match mode | One on one AI versus AI arena match. | Baseline | `top-shot-v0-1` | `src/main.js`, `src/state.js`, `src/systems.js` | Match starts, fighters spawn, fight progresses. | `npm run smoke:sim`, then full `npm run smoke`. | Start match mode and observe full engagement. | High | Protect during state and AI edits. |
...
| GP-005 | CQC Lab | Isolated manual and auto CQC test mode. | Strengthened coverage, needs run | `top-shot-anomaly-audit-2026-07-12` | `src/cqcLab.js`, `tests/cqcSmoke.js`, `src/main.js`, `src/three/actors3D.js` | Lab loads, actions trigger, auto and slow mode work, CQC state remains finite. | `npm run smoke:cqc`. | Run CQC Lab checklist. | High | Run deterministic strengthened smoke. Do not weaken if it reveals a real bug. |
| GP-006 | Body shots | Body shot actions affect zones and reactions. | Strengthened coverage, needs run | `top-shot-anomaly-audit-2026-07-12` | `src/cqcLab.js`, `tests/cqcSmoke.js`, `src/combat.js`, `src/three/actors3D.js` | Body zones are hit and recorded, visible reaction is readable. | `npm run smoke:cqc`. | Use body shot in CQC Lab. | Medium | Verify after smoke run. |
| GP-007 | Sweeps and trips | Fighter can be grounded by sweep or trip. | Strengthened deterministic coverage, needs run | `top-shot-anomaly-audit-2026-07-12` | `src/cqcLab.js`, `tests/cqcSmoke.js`, actors files | Fighter enters grounded state correctly under deterministic sweep setup. | `npm run smoke:cqc`. | Trigger sweep, verify grounded pose. | High | Verify after smoke run. |
| GP-008 | Mounting | Top fighter mounts without clipping through body. | Strengthened coverage, needs run | `top-shot-anomaly-audit-2026-07-12` | `src/cqcLab.js`, `tests/cqcSmoke.js`, `src/three/actors3D.js`, `src/three/effects3D.js` | Mount pose places fighter on body, not through body, and remains near but not centered inside the bottom fighter. | `npm run smoke:cqc`. | Trigger mount, rotate view if available, verify no clipping. | High | Verify after smoke run. |
| GP-009 | Mount escape | Mounted fighter can attempt escape. | Strengthened coverage, needs run | `top-shot-anomaly-audit-2026-07-12` | `src/cqcLab.js`, `tests/cqcSmoke.js`, combat files | Escape attempt changes state or pressure without corrupting CQC state. | `npm run smoke:cqc`. | Trigger mount escape in lab. | High | Verify after smoke run. |
| GP-010 | Limb grabs and disarms | CQC includes limb grab and disarm purpose where implemented. | Baseline | `top-shot-v0-1` | `src/cqcLab.js`, `src/combat.js` | Disarm changes weapon or control state. | `npm run smoke:cqc` now triggers both actions. | Trigger limb or disarm action. | High | Verify exact gameplay effect later. |
| GP-011 | Ranged fire | Gunfire and projectiles are represented. | Baseline | `top-shot-v0-1` | `src/systems.js`, `src/combat.js`, `src/three/effects3D.js` | Shots resolve and visible effects appear. | `npm run smoke:sim`. | Run ranged archetype match. | High | Do not break during effects work. |
...
| GP-013 | Damage and state integrity | Health, stamina, block, dodge, bleed, recovery caps, incapacitation, finite fighter/projectile state. | Known issue with mitigation branch | `top-shot-v0-1`; mitigation branch `top-shot-smoke-invalid-state-fix`; audit branch `top-shot-anomaly-audit-2026-07-12` | `src/state.js`, `src/systems.js`, `src/combat.js`, `src/explosives.js`, `tests/simSmoke.js` | Damage and dive/grenade simulation never create NaN position, health, elevation, projectile ttl, or projectile velocity. | `npm run smoke:sim`, then full `npm run smoke`. | Observe match until finish, especially suit operative versus survival commando grenade and suppression exchanges. | High | Run smoke on audit branch. |
| GP-014 | Fight ending | Fight can end or reach clear state. | Baseline | `top-shot-v0-1` | `src/systems.js`, `src/state.js` | Match resolves without infinite broken state. | `npm run smoke:sim`. | Watch full match. | High | Use diagnostic context if it fails. |
| GP-015 | Grenade and suppression dive safety | Dive movement must be finite whether caused by grenade dodge or suppression/evasion pose. | Needs verification | `top-shot-smoke-invalid-state-fix`, inherited by audit branch | `src/explosives.js`, `src/combat.js`, `src/systems.js`, `tests/simSmoke.js` | `diveT` without `diveVx`/`diveVy` does not corrupt fighter coordinates. Grenade values remain finite. | Direct regression added to `npm run smoke:sim`; `node --check` passed on patched file; full smoke still needed. | Force suppression and grenade pressure in match mode. | High | Run `npm run smoke:sim` first. |

## AI and simulation matrix

| ID | Area | Feature or purpose | Status | Branch source | Key files | Preservation target | Automated check | Manual QA | Risk | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AI-001 | Movement | AI fighters move through arena. | Baseline | `top-shot-v0-1` | `src/brain.js`, `src/perception.js`, `src/tactics.js`, `src/systems.js` | Fighters move with valid positions. | `npm run smoke:sim`, then full smoke. | Start match and track movement. | High | Run smoke after invalid-state fix. |
...
| AI-003 | Stealth and hiding | Stealth phases, shadow hiding, noise suspicion, sight memory, and search planning. | Corrected smoke expectation, needs run | `top-shot-anomaly-audit-2026-07-12`; PR #26 also reports related blocker | `src/stealth.js`, `src/hiding.js`, `tests/stealthSmoke.js`, `src/perception.js`, `src/tactics.js` | Hide, detection, last-known position, sound suspicion, and search states work. Tests allow valid immediate phase transitions but still require hard sight for alert. | `npm run smoke:stealth`. | Break line of sight and observe search. | High | Run corrected stealth smoke and port/rebase into PR #26 if needed. |
...
| AI-005 | Last seen investigation | AI can investigate last seen locations. | Baseline | `top-shot-v0-1` | `src/perception.js`, `src/stealth.js`, `src/brain.js`, `src/hiding.js` | Search behavior moves to meaningful target. | `npm run smoke:stealth` partly covers this. | Break line of sight and observe search. | Medium | Add explainable decision labels. |
...
| 3D-002 | Actor rigs | Segmented placeholder actors with readable body parts. | Baseline | `top-shot-v0-1` | `src/three/actors3D.js` | Head, torso, arms, legs remain readable top down. | `npm run smoke:model`. | View match and CQC Lab. | High | Replace only with tested equivalent. |
...
| 3D-008 | Camera readability | Top down camera remains primary and readable. | Baseline | `top-shot-v0-1` | `src/three/topShot3D.js`, `src/main.js`, `src/cameraAngles.js` | Camera follows action and does not lose fighters. | Build. | Start match and track fighters. | High | Preserve during camera drama work. |
...
| SS-005 | Tournament simulation | Repeated fights, wins, match length, damage, notable events. | Planned | Starshot roadmap | `src/sim/tournamentRunner.js` | Repeated runs produce valid logs without corrupting state. | Starshot | Implement after smoke failure is verified fixed. |
...
| DOC-010 | `top-shot/docs/AUDIT_REPORT_2026-07-12.md` | Records this full-game audit and what was fixed versus deferred. | Needs verification | Audit scope or findings change. | Missed instructions reappear. | Keep current until audit branch is resolved. |
...
| --- | --- | --- | --- | --- |
| BLK-001 | PR #5 documents `npm run smoke` failure with `Invalid fighter state`. Mitigation has been added on `top-shot-smoke-invalid-state-fix`, focused on unsafe dive velocity and grenade numeric state in `src/explosives.js`. `tests/simSmoke.js` now prints detailed state snapshots if a failure remains. | PR #5 body plus smoke-fix branch inspection | Stable merge is unsafe until full smoke passes. | Run `npm run smoke:sim` first, then `npm run check`, full `npm run smoke`, and `npm run build` on the audit branch or smoke-fix branch; if green, update PR #5 and this blocker. |
...
| BLK-004 | Strengthened deterministic CQC smoke may reveal true CQC regressions that old smoke missed. | Anomaly audit branch | Branch cannot be called stable until CQC preservation smoke passes. | Run `npm run smoke:cqc` and patch real failures instead of weakening coverage. |
| BLK-005 | PR #26 reports a stale stealth smoke expectation around immediate phase transition. This audit branch corrects `tests/stealthSmoke.js`, but the studio visual branch still needs verification or a rebase/port. | PR #26 and audit branch | Studio visual pipeline cannot be considered clean until stealth smoke is corrected and run there. | Run `npm run smoke:stealth` on the audit branch, then port/rebase into PR #26 if needed. |
