...
| GP-005 | CQC Lab | Isolated manual and auto CQC test mode. | Baseline | `top-shot-v0-1` | `src/cqcLab.js`, `src/main.js`, `src/three/actors3D.js` | Lab loads, actions trigger, auto and slow mode work. | `npm run smoke:cqc`. | Run CQC Lab checklist. | High | Keep isolated and testable. |
...
| GP-007 | Sweeps and trips | Fighter can be grounded by sweep or trip. | Baseline | `top-shot-v0-1` | `src/cqcLab.js`, `src/combat.js`, actors files | Fighter enters grounded state correctly. | `npm run smoke:cqc` where covered. | Trigger sweep, verify grounded pose. | High | Preserve before adding new animations. |
| GP-008 | Mounting | Top fighter mounts without clipping through body. | Baseline | `top-shot-v0-1` | `src/cqcLab.js`, `src/three/actors3D.js`, `src/three/effects3D.js` | Mount pose places fighter on body, not through body. | `npm run smoke:cqc` where covered. | Trigger mount, rotate view if available, verify no clipping. | High | Use as regression gate for animation work. |
| GP-009 | Mount escape | Mounted fighter can attempt escape. | Baseline | `top-shot-v0-1` | `src/cqcLab.js`, combat files | Escape attempt changes state or pressure. | `npm run smoke:cqc` where covered. | Trigger mount escape in lab. | High | Protect during grapple refactor. |
...
| GP-013 | Damage and state integrity | Health, stamina, block, dodge, bleed, recovery caps, incapacitation, finite fighter/projectile state. | Known issue with mitigation branch | `top-shot-v0-1`; mitigation branch `top-shot-smoke-invalid-state-fix` | `src/state.js`, `src/systems.js`, `src/combat.js`, `src/explosives.js`, `tests/simSmoke.js` | Damage and dive/grenade simulation never create NaN position, health, elevation, projectile ttl, or projectile velocity. | `npm run smoke:sim`, then full `npm run smoke`. | Observe match until finish, especially suit operative versus survival commando grenade and suppression exchanges. | High | Run smoke on mitigation branch. |
