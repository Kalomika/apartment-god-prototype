...
| GP-013 | Damage and state integrity | Health, stamina, block, dodge, bleed, recovery caps, incapacitation, finite fighter/projectile state. | Known issue with mitigation branch | `top-shot-v0-1`; mitigation branch `top-shot-smoke-invalid-state-fix` | `src/state.js`, `src/systems.js`, `src/combat.js`, `src/explosives.js` | Damage and dive/grenade simulation never create NaN position, health, elevation, projectile ttl, or projectile velocity. | `npm run smoke`, currently needs rerun after mitigation. | Observe match until finish, especially suit operative versus survival commando grenade and suppression exchanges. | High | Run smoke on mitigation branch. |
...
| GP-015 | Grenade and suppression dive safety | Dive movement must be finite whether caused by grenade dodge or suppression/evasion pose. | Needs verification | `top-shot-smoke-invalid-state-fix` | `src/explosives.js`, `src/combat.js`, `src/systems.js` | `diveT` without `diveVx`/`diveVy` does not corrupt fighter coordinates. Grenade values remain finite. | `node --check` passed on patched file; smoke still needed. | Force suppression and grenade pressure in match mode. | High | Run full smoke suite and browser match. |

## AI and simulation matrix

| ID | Area | Feature or purpose | Status | Branch source | Key files | Preservation target | Automated check | Manual QA | Risk | Next action |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| AI-001 | Movement | AI fighters move through arena. | Baseline | `top-shot-v0-1` | `src/brain.js`, `src/perception.js`, `src/tactics.js`, `src/systems.js` | Fighters move with valid positions. | Smoke, currently needs mitigation rerun. | Start match and track movement. | High | Run smoke after invalid-state fix. |
