...
| WF-010 | Whole-game anomaly audit | Audit docs, smoke tests, CQC preservation, playable links, and core runtime entry points for missed instructions. | Needs verification | `top-shot-anomaly-audit-2026-07-12` | `docs/AUDIT_REPORT_2026-07-12.md`, docs, `tests/cqcSmoke.js` | Audit report exists, docs encode playable-link rule, CQC smoke guards preserved behavior. | High | Run strengthened smoke commands and patch any true failures. |
...
| GP-007 | Sweeps and trips | Fighter can be grounded by sweep or trip. | Strengthened coverage, needs run | `top-shot-anomaly-audit-2026-07-12` | `src/cqcLab.js`, `tests/cqcSmoke.js`, actors files | Fighter enters grounded state correctly or visibly destabilizes through repeated attempts. | `npm run smoke:cqc`. | Trigger sweep, verify grounded pose. | High | Verify after smoke run. |
...
| AI-003 | Stealth and hiding | Ninja or shadow behavior can hide and search. | Baseline | `top-shot-v0-1` | `src/stealth.js`, `src/perception.js`, `src/tactics.js` | Hide, detection, and search states work. | `npm run smoke:stealth`. | Break line of sight and observe search. | High | Add debug overlay labels before deep AI changes. |
...
| DOC-010 | `top-shot/docs/AUDIT_REPORT_2026-07-12.md` | Records this anomaly audit and what was fixed versus deferred. | Needs verification | Audit scope or findings change. | Missed instructions reappear. | Keep current until audit branch is resolved. |
...
| BLK-004 | Strengthened CQC smoke may reveal true CQC regressions that old smoke missed. | Anomaly audit branch | Branch cannot be called stable until CQC preservation smoke passes. | Run `npm run smoke:cqc` and patch real failures instead of weakening coverage. |
